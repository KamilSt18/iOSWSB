...
//  AlertControllerTest
//
//  Created by Student G225 08 on 04/04/2022.
//
...
class ViewController: UIViewController {
    ...
    textField.placeholder = "Tag" }
        alertController.addAction(confirmAction); alertController.addAction(cancelAction); self.present(alertController, animated: true, completion: nil)
    }
    
    @IBAction func showToast(_sender: Any) {
    let toastLabel = UILabel(frame: CGRect(x: self.view.frame.size.width/2 - 75, y: self.view.frame.size.height-100, width: 150, height: 35))
        toastLabel.backgroundColor = UIColor.black.withAlphaComponent(0.6); toastLabel.textColor = UIColor.white
    toastLabel.font = .systemFont(ofSize: 12.0)
    toastLabel.textAlignment = .center;
    toastLabel.text = "Widze Cię!"
